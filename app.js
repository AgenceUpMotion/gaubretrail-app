...
const mapSearch=document.querySelector('.volunteer-finder');
const mapScrim=document.querySelector('#mobileMapScrim');
const searchButton=document.querySelector('#mobileSearch');
const optionsButton=document.querySelector('#mobileOptions');
let mobileMapTrigger=null;
const updateMobileMapControls=()=>{
  const optionsOpen=!mapPanel.classList.contains('closed');
  const searchOpen=mapSearch.classList.contains('is-open');
  optionsButton.setAttribute('aria-expanded',String(optionsOpen));
  searchButton.setAttribute('aria-expanded',String(searchOpen));
  mapScrim.hidden=!optionsOpen&&!searchOpen;
};
const closeMobileMapPanels=(restoreFocus=true)=>{mapPanel.classList.add('closed');mapSearch.classList.remove('is-open');updateMobileMapControls();if(restoreFocus&&mobileMapTrigger&&matchMedia('(max-width: 800px)').matches)mobileMapTrigger.focus();mobileMapTrigger=null;};
const toggleMapOptions=()=>{const open=mapPanel.classList.contains('closed');closeMobileMapPanels(false);mapPanel.classList.toggle('closed',!open);if(open){mobileMapTrigger=optionsButton;document.querySelector('#closeMapOptions').focus();}updateMobileMapControls();};
document.querySelector('#menuButton').addEventListener('click',toggleMapOptions);
optionsButton.addEventListener('click',toggleMapOptions);
searchButton.addEventListener('click',()=>{const open=!mapSearch.classList.contains('is-open');closeMobileMapPanels(false);mapSearch.classList.toggle('is-open',open);if(open)mobileMapTrigger=searchButton;updateMobileMapControls();if(open)document.querySelector('#publicVolunteerSearch').focus();});
document.querySelector('#mobileCourses').addEventListener('click',()=>{closeMobileMapPanels(false);document.querySelector('#coursePicker')?.click();});
document.querySelector('#closeMapOptions').addEventListener('click',closeMobileMapPanels);
document.querySelector('#closeMapSearch').addEventListener('click',closeMobileMapPanels);
mapScrim.addEventListener('click',closeMobileMapPanels);
document.querySelector('#publicVolunteerResults').addEventListener('click',event=>{if(event.target.closest('[data-missions]'))closeMobileMapPanels(false);});
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!mapScrim.hidden)closeMobileMapPanels();});
